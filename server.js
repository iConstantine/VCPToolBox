...
async function startServer() {
  ...
  server = app.listen(port, () => {
    // 禁用 Node.js HTTP server 默认超时，防止 SSE 长连接 / Claude extended thinking 长推理被截断
    // 默认 requestTimeout=300s 会在 VCP 等上游响应头期间（不发任何数据给客户端时）触发 socket 销毁
    // 导致 client_gone → 输入 tokens 白烧。设为 0 = 无限等待（由应用层 connectionTimeout 控制）。
    server.requestTimeout = 0;
    server.headersTimeout = 120000; // 收请求头的超时保持合理值（120s），只放宽响应超时
    server.keepAliveTimeout = 30000; // 空闲 keep-alive 连接超时 30s
