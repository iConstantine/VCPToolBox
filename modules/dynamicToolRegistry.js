...
const PRIVATE_CONFIG_RELATIVE_PATH = path.join(
  "Plugin",
  "DynamicToolBridge",
  "config.env"
);
...
function truncateToTokenBudget(value, maxTokens) {
  ...
  return cleanText(
    `${pieces.slice(0, budget).join(" ")}...`,
    Math.max(24, budget * 14)
  );
}
...
function mergeConfig(base, fileConfig, overrideConfig) {
  ...
  merged.manualOverrides = {
    ...
    ...(fileConfig && fileConfig.manualOverrides
      ? fileConfig.manualOverrides
      : {}),
    ...(overrideConfig && overrideConfig.manualOverrides
      ? overrideConfig.manualOverrides
      : {}),
  };
  merged.smallModel = {
    ...cloneJson(base.smallModel),
    ...(fileConfig && fileConfig.smallModel ? fileConfig.smallModel : {}),
    ...(overrideConfig && overrideConfig.smallModel
      ? overrideConfig.smallModel
      : {}),
  };
  delete merged.smallModel.apiKey;
  delete merged.smallModel.apiKeyEnv;

  merged.maxBriefListItems = clampInteger(
    merged.maxBriefListItems,
    1,
    500,
    DEFAULT_CONFIG.maxBriefListItems
  );
  merged.maxExpandedPlugins = clampInteger(
    merged.maxExpandedPlugins,
    0,
    50,
    DEFAULT_CONFIG.maxExpandedPlugins
  );
  merged.maxForcedCategoryPlugins = clampInteger(
    merged.maxForcedCategoryPlugins,
    1,
    100,
    DEFAULT_CONFIG.maxForcedCategoryPlugins
  );
  merged.maxInjectionChars = clampInteger(
    merged.maxInjectionChars,
    1000,
    120000,
    DEFAULT_CONFIG.maxInjectionChars
  );
  merged.classificationDebounceMs = clampInteger(
    merged.classificationDebounceMs,
    0,
    60000,
    DEFAULT_CONFIG.classificationDebounceMs
  );
  merged.classifierTimeoutMs = clampInteger(
    merged.classifierTimeoutMs,
    100,
    120000,
    DEFAULT_CONFIG.classifierTimeoutMs
  );
...
class DynamicToolRegistry {
  constructor() {
    ...
    this.catalogPath = path.join(
      this.toolConfigsDir,
      "dynamic_tool_catalog.json"
    );
    this.categoriesPath = path.join(
      this.toolConfigsDir,
      "dynamic_tool_categories.json"
    );
    this.configPath = path.join(
      this.toolConfigsDir,
      "dynamic_tool_bridge.config.json"
    );
    this.privateConfigPath = path.join(
      this.projectBasePath,
      PRIVATE_CONFIG_RELATIVE_PATH
    );
    this.privateConfig = {};
  ...
  async initialize(options = {}) {
    ...
    await this._writeConfigIfMissingOrSanitized(
      fileConfig,
  ...
  /**
   * Remove stale catalog entries for distributed tools.
   *
   * When a distributed (cloud) server reconnects it receives a brand-new
   * serverId (generated from a timestamp + random suffix), which produces a
   * new originKey (`distributed:${serverId}:${pluginName}`).  The previous
   * catalog entry for the same logical tool still sits in the catalog with
   * `online: false`.  Over repeated reconnections this causes unbounded
   * accumulation of dead entries.
   *
   * This method detects that situation: for every distributed tool that is
   * currently online (just registered), it finds old catalog entries that
   * share the same `pluginName` but have a different `originKey` and removes
   * them.
   *
   * @param {Array} currentRecords – records produced by _extractRecords this sync
   */
    ...
    const manifestRecords = this._extractRecords(
      {
        plugins: new Map(
          asArray(manifests)
            .filter((manifest) => manifest && manifest.name)
            .map((manifest) => [
              manifest.name,
              {
                ...manifest,
    ...
    this.classificationPromise = (async () => {
      while (this.classificationQueue.size > 0) {
        ...
        for (const item of items) {
          ...
          if (!current || current.sourceHash !== item.record.sourceHash)
            continue;
          try {
            const classification = await this._classifyRecord(
              current,
              item.reason
            );
            this.categories.set(current.originKey, {
              ...
              categories: asArray(classification.categories)
                .map(String)
                .filter(Boolean),
              keywords: asArray(classification.keywords)
                .map(String)
                .filter(Boolean),
  ...
  async buildInjection(options = {}) {
    ...
    if (available.length === 0)
      return "Dynamic VCP Tools: no currently available tools.";
    ...
    for (const toolName of directives.tools) {
      const match = available.find((record) =>
        this._matchesToolName(record, toolName)
      );
    ...
    for (const categoryName of directives.categories) {
      const matches = available
        .filter((record) =>
          this._recordCategories(record).some((category) =>
            this._categoryMatches(category, categoryName)
          )
        )
      ...
      if (matches.length > allowed.length) {
        notices.push(
          `Category "${categoryName}" has ${
            matches.length - allowed.length
          } more tools hidden by maxForcedCategoryPlugins.`
        );
      }
    }
    ...
    for (const item of scored) {
      if (
        expandedKeys.size >=
        this.config.maxExpandedPlugins +
          directives.tools.size +
          directives.categories.size * this.config.maxForcedCategoryPlugins
      )
        break;
      ...
      if (
        Array.from(expandedKeys).filter(
          (key) => !this._isForcedKey(key, directives, available)
        ).length >= this.config.maxExpandedPlugins
      )
        break;
    }
    ...
    lines.push(
      "Light list: names and short descriptions are always shown; full usage is expanded only for matched or explicitly requested tools."
    );
    lines.push("");
    ...
    for (const record of briefRecords) {
      ...
      const brief = this._briefForLightList(
        record,
        categories
          .split(",")
          .map((item) => item.trim())
      ...
      lines.push(
        `- ${this._formatLightListName(record)} [${categories}]: ${brief}`
      );
    }
    if (available.length > briefRecords.length) {
      lines.push(
        `- ... ${
          available.length - briefRecords.length
        } more tools hidden by maxBriefListItems.`
      );
    }
    ...
    if (expandedRecords.length > 0) {
      ...
      for (const record of expandedRecords) {
        lines.push(
          `--- ${record.displayName || record.pluginName} (${
            record.pluginName
          }) ---`
        );
        lines.push(
  ...
  getAdminState() {
    const records = this._getAdminRecords().map((record) => {
      const classification = this.categories.get(record.originKey);
      return {
    ...
    return {
      ...
      isClassifying: Boolean(
        this.classificationPromise ||
          this.classificationTimer ||
          this.classificationQueue.size > 0
      ),
  ...
  async updateConfig(nextConfig = {}) {
    this.persistedConfig = mergeConfig(
      DEFAULT_CONFIG,
      this.persistedConfig,
      nextConfig
    );
    ...
    await this._queueWrite(
      this.configPath,
      this._redactConfig(this.persistedConfig)
    );
  ...
  async forceRebuild(options = {}) {
    const mode =
      typeof options === "string" ? options : options.mode || "classification";
    ...
    if (wait) {
      ...
      classificationPromise.catch((error) => {
        this.lastError = error.message;
        console.error(
          "[DynamicToolRegistry] manual rebuild classification failed:",
          error
        );
      });
    }
  ...
  _getAdminRecords() {
    ...
    const availableIdentityKeys = new Set(
      records
        .filter(
          (record) =>
            record?.originKind === "distributed" &&
            this._isAvailable(record) &&
    ...
    return records
      .filter((record) => {
        ...
        if (this._isAvailable(record) && record.available !== false)
          return true;
  ...
  _extractRecords(pluginManager, now) {
    const descriptions =
      typeof pluginManager.getIndividualPluginDescriptions === "function"
    ...
    for (const manifest of pluginManager.plugins.values()) {
      ...
      const originId = manifest.isDistributed
        ? manifest.serverId || "unknown"
        : "local";
      const originKey =
        originKind === "distributed"
          ? `distributed:${originId}:${manifest.name}`
          : `local:${manifest.name}`;
      const fullDescription =
        descriptions.get(`VCP${manifest.name}`) ||
        this._buildFullDescriptionFromManifest(manifest);
      ...
      const record = {
        ...
        descriptionHash,
        sourceHash,
        commandIdentifiers: commands
          .map((cmd) => cmd.commandIdentifier || cmd.command || manifest.name)
          .filter(Boolean),
        brief: cleanText(
          manifest.description ||
            commands.map((cmd) => cmd.description).find(Boolean) ||
            ""
        ),
        fullDescription,
        lastSeenAt: now,
  ...
  _buildFullDescriptionFromManifest(manifest) {
    ...
    for (const command of commands) {
      const identifier =
        command.commandIdentifier || command.command || manifest.name;
      chunks.push(
      ...
      if (command.description)
        chunks.push(`  ${String(command.description).trim()}`);
      if (command.example)
  ...
  _isAvailable(record) {
    const excluded = new Set(
      asArray(this.config.manualOverrides?.excludedOriginKeys)
    );
    return (
  ...
  async _classifyWithSmallModel(record, reason) {
    ...
    try {
      const response = await withTimeout(
        fetch(requestConfig.endpoint, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "You classify tool plugins. Return JSON only.",
              },
        ...
        this.config.classifierTimeoutMs,
        "small model classification request"
      );
      ...
      return this._normalizeClassification(
        JSON.parse(jsonText),
        record,
        "small_model"
      );
      ...
      if (this.debugMode)
        console.warn(
          "[DynamicToolRegistry] small model classification failed:",
          error.message
        );
      return null;
    }
  ...
  _resolveSmallModelRequestConfig(smallModel = {}) {
    ...
    const apiKey = useMainConfig
      ? process.env.API_Key || ""
      : smallModel.apiKey ||
        (smallModel.apiKeyEnv ? process.env[smallModel.apiKeyEnv] : "");
    return {
      endpoint,
      apiKey,
  ...
  _normalizeClassification(result, record, classifiedBy) {
    ...
    const categories = asArray(result.categories)
      ...
      .map((item) => item.trim())
      .filter(Boolean);
    const keywords = asArray(result.keywords)
      .map(String)
      .map((item) => item.trim())
      .filter(Boolean);
  ...
  async _classifyWithEmbeddings(record) {
    ...
    try {
      const text = cleanText(
        `${record.pluginName} ${record.displayName} ${record.description} ${record.fullDescription}`,
        2000
      );
      ...
      if (!Array.isArray(pluginVector) || pluginVector.length === 0)
        return null;

      const scored = [];
      for (const rule of CATEGORY_RULES) {
        const categoryVector = await this._getCategoryEmbedding(
          rule,
          getEmbedding
        );
      ...
      return {
        brief: this._compactBrief(
          ...
          record.brief ||
            record.description ||
            `${record.pluginName} provides a VCP tool.`
        ),
        categories,
      ...
      if (this.debugMode)
        console.warn(
          "[DynamicToolRegistry] RAG embedding classification failed:",
          error.message
        );
      return null;
    }
  ...
  _resolveEmbeddingProvider() {
    ...
    const vectorDBManager =
      this.pluginManager?.vectorDBManager || ragPlugin.vectorDBManager;
    if (
      vectorDBManager &&
      typeof vectorDBManager.getPluginDescriptionVector === "function"
    ) {
      return async (text) =>
        vectorDBManager.getPluginDescriptionVector(
  ...
  _fallbackClassify(record) {
    const text =
      `${record.pluginName} ${record.displayName} ${record.description} ${record.fullDescription}`.toLowerCase();
    const categories = [];
    ...
    const brief = this._compactBrief(
      record,
      selectedCategories,
      record.brief ||
        record.description ||
  ...
  _formatLightListName(record) {
    ...
    if (displayName && displayName !== pluginName)
  ...
  _briefTokenBudget(record, categories = []) {
    ...
    const used = estimateTokenCount(
      `${this._formatLightListName(record)} ${categoryText}`
    );
    return Math.max(
  ...
  _compactBrief(record, categories, brief) {
    const fallback =
      record?.description ||
      `${record?.pluginName || "This tool"} provides a VCP tool.`;
    const source = brief || record?.brief || fallback;
    return truncateToTokenBudget(
      source,
      this._briefTokenBudget(record, categories)
    );
  }
  ...
  _parseDirectives(text) {
    ...
    const strongVerb =
      /(expand|expose|show|list|display|full|details|完整|全部|展开|暴露|显示|列出|详情)/i;
    ...
    for (const record of available) {
      for (const category of this._recordCategories(record))
        knownCategories.add(category);
    }
    for (const category of knownCategories) {
      if (lower.includes(category.toLowerCase()))
        directives.categories.add(category);
    }
    for (const record of available) {
      const names = [
        record.pluginName,
        record.displayName,
        ...(record.commandIdentifiers || []),
      ].filter(Boolean);
  ...
  _scoreRecord(record, queryText, directives) {
    ...
    if (
      directives.tools.has(record.pluginName) ||
      Array.from(directives.tools).some((name) =>
        this._matchesToolName(record, name)
      )
    ) {
      score += 100;
    }
    if (
      Array.from(directives.categories).some((name) =>
        this._recordCategories(record).some((category) =>
          this._categoryMatches(category, name)
        )
      )
    ) {
      score += 80;
    ...
    const keywords = asArray(classification?.keywords).map((item) =>
      item.toLowerCase()
    );
    for (const category of categories) {
      if (
        queryLower.includes(category.toLowerCase()) ||
        queryTokens.has(category.toLowerCase())
      )
        score += 8;
    }
    for (const keyword of keywords) {
      if (
        ...
        queryTokens.has(keyword.toLowerCase())
      )
        score += 5;
    }
    const haystack = `${record.pluginName} ${record.displayName} ${
      record.description
    } ${classification?.brief || ""}`.toLowerCase();
  ...
  _isForcedKey(originKey, directives, records) {
    ...
    if (
      ...
      return true;
    return Array.from(directives.categories).some((name) =>
      this._recordCategories(record).some((category) =>
        this._categoryMatches(category, name)
      )
    );
  ...
  _compareRecordsWithPinned(a, b) {
    const pinned = new Set(
      asArray(this.config.manualOverrides?.pinnedOriginKeys)
    );
  ...
  _compareRecords(a, b) {
    ...
    const displayCompare = String(a.displayName || a.pluginName).localeCompare(
      String(b.displayName || b.pluginName)
    );
  ...
  _truncateInjection(text) {
    ...
    return `${String(text)
      .slice(0, Math.max(0, max - suffix.length))
      .trimEnd()}${suffix}`;
  }
  ...
  _scheduleClassificationFlush() {
    ...
    this.classificationTimer = setTimeout(() => {
      this.flushClassificationQueue().catch((error) => {
        this.lastError = error.message;
        console.error(
          "[DynamicToolRegistry] classification flush failed:",
          error
        );
      });
    }, delay);
    if (delay === 0 && this.classificationTimer.unref)
      this.classificationTimer.unref();
  }

  async _loadCatalog() {
    const data = await this._readJson(this.catalogPath, {
      version: 1,
      snapshotId: 0,
      plugins: {},
    });
  ...
  async _loadCategories() {
    const data = await this._readJson(this.categoriesPath, {
      version: 1,
      items: {},
    });
  ...
  async _writeConfigIfMissingOrSanitized(
    fileConfig,
    configToWrite = this.persistedConfig
  ) {
    const shouldWrite =
      !fileConfig ||
      JSON.stringify(this._redactConfig(fileConfig)) !==
        JSON.stringify(this._redactConfig(configToWrite));
    if (shouldWrite) {
      await this._queueWrite(
        this.configPath,
        this._redactConfig(configToWrite)
      );
    }
  }
  ...
  async _readJson(filePath, fallback) {
    try {
      ...
      if (error.code !== "ENOENT") {
        this.lastError = `Failed to read ${path.basename(filePath)}: ${
          error.message
        }`;
        if (this.debugMode)
          console.warn(`[DynamicToolRegistry] ${this.lastError}`);
      }
      return fallback;
    }
  }
  ...
  async _readPrivatePluginConfig() {
    try {
      ...
      if (
        Object.prototype.hasOwnProperty.call(env, "SmallModel_Use_Main_Config")
      ) {
        smallModel.useMainConfig = parseEnvBoolean(
          env.SmallModel_Use_Main_Config,
          true
        );
      }
      if (env.SmallModel_Endpoint)
        smallModel.endpoint = env.SmallModel_Endpoint;
      ...
      if (env.SmallModel_API_Key_Env)
        smallModel.apiKeyEnv = env.SmallModel_API_Key_Env;
      return Object.keys(smallModel).length > 0 ? { smallModel } : {};
    } catch (error) {
      if (error.code !== "ENOENT") {
        this.lastError = `Failed to read ${path.basename(
          this.privateConfigPath
        )}: ${error.message}`;
        if (this.debugMode)
          console.warn(`[DynamicToolRegistry] ${this.lastError}`);
      }
  ...
  _bindPluginManagerEvents(pluginManager) {
    ...
    if (
      this._boundPluginManager &&
      typeof this._boundPluginManager.removeListener === "function"
    ) {
      if (this._toolsChangedHandler)
        this._boundPluginManager.removeListener(
          "tools_changed",
          this._toolsChangedHandler
        );
      if (this._distributedOfflineHandler)
        this._boundPluginManager.removeListener(
          "distributed_tools_offline",
          this._distributedOfflineHandler
        );
    }

    this._toolsChangedHandler = (payload = {}) => {
      this.syncFromPluginManager(payload.reason || "tools_changed").catch(
        (error) => {
          this.lastError = error.message;
          console.error(
            "[DynamicToolRegistry] tools_changed sync failed:",
            error
          );
        }
      );
    ...
    this._distributedOfflineHandler = (payload = {}) => {
      this.markDistributedOffline(
        payload.serverId,
        payload.manifests || payload.tools || []
      ).catch((error) => {
        this.lastError = error.message;
        console.error(
          "[DynamicToolRegistry] distributed offline sync failed:",
          error
        );
      });
    };
