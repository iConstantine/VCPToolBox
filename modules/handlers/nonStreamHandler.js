...
function hasReasoningOnlySignal(message, choice) {
  ...
  const hasReasoningField = (obj) =>
    obj &&
    typeof obj === "object" &&
    reasoningKeys.some((key) => hasVisibleContent(obj[key]));
  return (
    hasReasoningField(message) ||
    hasReasoningField(choice) ||
    hasReasoningField(choice?.delta)
  );
}
...
function isReasoningOnlyNonStreamResponse(rawResponseText) {
  const { parsedJson, message, choice } =
    parseNonStreamResponse(rawResponseText);
  if (!parsedJson || !message || hasToolOrRefusalPayload(message)) return false;
  return (
    !hasVisibleContent(message.content) &&
    hasReasoningOnlySignal(message, choice)
  );
}
...
async function readNonStreamResponseWithSemanticRetry({
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (attempt >= maxAttempts - 1) {
      if (debugMode) {
        console.warn(
          `[NonStream Semantic Retry] ${label}: reasoning-only response detected, but semantic retries are exhausted. Returning last response.`
        );
      }
      return {
        response,
    ...
    if (debugMode) {
      console.warn(
        `[NonStream Semantic Retry] ${label}: upstream returned reasoning-only response without visible content. Retrying in ${currentDelay}ms... (${
          attempt + 1
        }/${maxAttempts})`
      );
    }
...
class NonStreamHandler {
  ...
  async handle(req, res, firstAiAPIResponse) {
    ...
    const containsImageUrlPart = (content) =>
    ...
    const maybeTranslateToolPayloadMedia = async (content) => {
      ...
      const shouldTranslateToolMedia =
        shouldProcessMedia || isTextOnlyForceTranslateModel;
      ...
      if (!pluginManager.messagePreprocessors.has(processorName)) {
        if (DEBUG_MODE)
          console.warn(
            `[VCP NonStream Loop] Tool payload contains image_url, but ${processorName} is unavailable. Forwarding original payload.`
          );
        return content;
      }

      const originalImageParts = content.filter(
        (part) =>
          part?.type === "image_url" &&
          part.image_url &&
          typeof part.image_url.url === "string"
      ...
      if (DEBUG_MODE) {
        console.log(
          `[VCP NonStream Loop] Translating tool-returned image_url content via ${processorName}. textOnly=${!!isTextOnlyForceTranslateModel}, plus=${!!shouldProcessMediaPlus}`
        );
      }
      ...
      try {
        ...
        console.error(
          `[VCP NonStream Loop] Error translating tool-returned media via ${processorName}:`,
          pluginError
        );
        return content;
      }
      ...
      if (shouldProcessMediaPlus && !isTextOnlyForceTranslateModel) {
        const translatedWithoutImages = translatedContent.filter(
    ...
    const fetchNonStreamCompletion = (body, label) =>
      fetchWithRetry(
    ...
    const recordOneRingAIResponse = (aiText, phaseLabel) => {
      ...
      const recordPromise =
        oneRingResponseMeta &&
        typeof oneRingModule.recordAIResponseWithMeta === "function"
          ...
          ? oneRingModule.recordAIResponseFromMessages(
      ...
      if (recordPromise && typeof recordPromise.catch === "function") {
        recordPromise.catch((e) =>
          console.error(
            `[OneRing NonStream] Error recording AI response (${phaseLabel}):`,
            e
          )
        );
      }
    ...
    const extractedMessage = (rawResponseText) =>
      parseNonStreamResponse(rawResponseText).message;
    ...
    if (writeChatLog)
      chatLogs.push({
        ...
        response: initMessage || fullContentFromAI,
      });
    ...
    do {
      ...
      if (abortController && abortController.signal.aborted) {
        if (DEBUG_MODE)
          console.log("[VCP NonStream Loop] Abort detected, exiting loop.");
        break;
      }
      ...
      const toolCalls = vcpToolUseForbidden
        ? []
        : ToolCallParser.parse(currentAIContentForLoop);

      if (toolCalls.length > 0) {
        anyToolProcessedInCurrentIteration = true;
        const { normal: normalCalls, archery: archeryCalls } =
          ToolCallParser.separate(toolCalls);
        const archeryErrorContents = [];
        const archeryStatusSummaryItems = [];

        // 执行 Archery 调用
        const archeryLogs = await Promise.all(
          archeryCalls.map(async (toolCall) => {
            try {
              ...
              return {
                tool: toolCall,
                result: [{ type: "text", text: String(e.message) }],
              };
            }
          })
        );
        ...
        if (normalCalls.length === 0 && archeryErrorContents.length > 0) {
          let assistantMessages = [
            { role: "assistant", content: currentAIContentForLoop },
          ];
          ...
          const errorPayload = `<!-- VCP_TOOL_PAYLOAD -->\n${JSON.stringify(
            archeryErrorContents
          )}`;
          currentMessagesForNonStreamLoop.push({
            role: "user",
          ...
          const recursionBody = {
            ...originalBody,
            messages: currentMessagesForNonStreamLoop,
            stream: false,
          };
          const recursionReadResult =
            await readNonStreamResponseWithSemanticRetry({
              fetchResponse: () =>
                fetchNonStreamCompletion(
                  recursionBody,
                  `archery_error_depth_${recursionDepth}`
                ),
              retries: apiRetries,
              delay: apiRetryDelay,
              debugMode: DEBUG_MODE,
          ...
          if (recursionAiResponse.ok) {
            ...
            handleDiaryFromAIResponse(recursionText).catch((e) =>
              console.error(
                `[VCP NonStream Loop] Error in diary handling for depth ${recursionDepth}:`,
                e
              )
            );
        ...
        const combinedToolResultsForAI = toolResults
          .map((r) => r.content)
          .flat();
        ...
        const toolResultsTextForRAG = JSON.stringify(
        ...
        if (RAGMemoRefresh) {
          currentMessagesForNonStreamLoop = await _refreshRagBlocksIfNeeded(
            currentMessagesForNonStreamLoop,
            {
        ...
        const hasImage = combinedToolResultsForAI.some(
        ...
        const recursionReadResult =
          await readNonStreamResponseWithSemanticRetry({
            fetchResponse: () =>
              fetchNonStreamCompletion(
                recursionBody,
                `tool_loop_depth_${recursionDepth}`
              ),
            retries: apiRetries,
            delay: apiRetryDelay,
            debugMode: DEBUG_MODE,
    ...
    try {
      ...
      if (!finalJsonResponse.choices?.[0]?.message) {
        finalJsonResponse.choices = [
          { message: { content: finalContentForClient } },
        ];
      } else {
        finalJsonResponse.choices[0].message.content = finalContentForClient;
      }
      finalJsonResponse.choices[0].finish_reason =
        recursionDepth >= maxRecursion ? "length" : "stop";
    } catch (e) {
      finalJsonResponse = {
        choices: [
          {
