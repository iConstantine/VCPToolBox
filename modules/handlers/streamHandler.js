...
class StreamHandler {
  ...
  async handle(req, res, firstAiAPIResponse) {
    ...
    let currentMessagesForLoop = originalBody.messages
    ...
    const recordOneRingAIResponse = (aiText, phaseLabel) => {
      ...
      if (recordPromise && typeof recordPromise.catch === "function") {
        recordPromise.catch((e) =>
          console.error(
            `[OneRing Stream] Error recording AI response (${phaseLabel}):`,
            e
          )
        );
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        const resetChunkIdleTimer = () => {
          ...
          chunkIdleTimer = setTimeout(() => {
            ...
            console.warn(
              `[Stream IdleTimeout] No upstream chunk received for ${
                CHUNK_IDLE_TIMEOUT / 1000
              }s. Assuming stream stalled. Forcing end.`
            );
            streamAborted = true;
            // 通知前端这次流异常结束
            if (!res.writableEnded && !res.destroyed) {
        ...
        const abortHandler = () => {
          ...
          if (DEBUG_MODE)
            console.log(
              "[Stream Abort] Abort signal received, stopping stream processing."
            );
          if (abortController?.signal)
            abortController.signal.removeEventListener("abort", abortHandler);
          if (aiResponse.body && !aiResponse.body.destroyed)
    ...
    // --- 初始 AI 调用 ---
    if (DEBUG_MODE)
      console.log("[VCP Stream Loop] Processing initial AI call.");
    let initialAIResponseData = await processAIResponseStreamHelper(
      firstAiAPIResponse,
      true
    );
    ...
    if (writeChatLog)
      chatLogs.push({
        ...
        response: initialAIResponseData.message,
      });
    ...
    while (recursionDepth < maxRecursion) {
      ...
      if (abortController && abortController.signal.aborted) {
        if (DEBUG_MODE)
          console.log("[VCP Stream Loop] Abort detected, exiting loop.");
        break;
      }
      ...
      if (toolCalls.length === 0) {
        if (DEBUG_MODE)
          console.log("[VCP Stream Loop] No tool calls found. Exiting loop.");
        if (!res.writableEnded) {
          ...
          try {
            ...
            console.error(
              "[VCP Stream Loop] Failed to write final chunk:",
              writeError.message
            );
            if (!res.writableEnded && !res.destroyed)
              try {
      ...
      const archeryLogs = await Promise.all(
        archeryCalls.map(async (toolCall) => {
          try {
            ...
            console.error(
              `[VCP Stream Loop Archery Error] ${toolCall.name}:`,
              e
            );
            return {
              tool: toolCall,
      ...
      if (normalCalls.length === 0 && archeryErrorContents.length > 0) {
        ...
        const nextAiAPIResponse = await fetchWithRetry(
          `${apiUrl}/v1/chat/completions`,
          {
            ...
            body: JSON.stringify({
              ...originalBody,
              messages: currentMessagesForLoop,
              stream: true,
            }),
        ...
        if (nextAiAPIResponse.ok) {
          let nextAIResponseData = await processAIResponseStreamHelper(
            nextAiAPIResponse,
            false
          );
      ...
      if (RAGMemoRefresh) {
        currentMessagesForLoop = await _refreshRagBlocksIfNeeded(
          currentMessagesForLoop,
          {
      ...
      handleDiaryFromAIResponse(nextAIResponseData.raw).catch((e) =>
        console.error(
          `[VCP Stream Loop] Error in diary handling for depth ${recursionDepth}:`,
          e
        )
      );
