...
=======
  // Quote CLI path to handle spaces (e.g. "C:\Program Files\Obsidian\Obsidian.com")
  const cliPath = OBSIDIAN_CLI.includes(" ")
    ? `"${OBSIDIAN_CLI}"`
    : OBSIDIAN_CLI;
  const parts = [cliPath];
  if (vault) {
...
const COMMAND_HANDLERS = {
  ...
  tasks: (args) => {
    ...
    const validScopes = ["daily", "all", "file"];
    if (args.scope && !validScopes.includes(args.scope)) {
      return error("scope invalid: " + args.scope + ". Use: daily, all");
    }
    const scope = args.scope || (args.file || args.path ? "file" : "daily");
    const flags = [];
  ...
  daily_prepend: (args) => {
    ...
    const result = execCLI(cmd);
    if (!result.success)
      return error("向每日笔记开头写入失败: " + result.output);
    return success(
      "已向今日每日笔记开头写入内容。" +
        (result.output ? "\n" + result.output : "")
    );
